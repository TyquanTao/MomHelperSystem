...
public class AnswerServiceImpl implements AnswerService {
    ...
    public JsonResult addAnswer(Answer answer) {
        ...
        Integer rows = answerMapper.updateQuestionReply(answer);
        if (rows == 0) {
            return new JsonResult(409,"回答问题成功，但数据未更新");
        }
...
//        }
    ...
    @Override
    public JsonResult cancelPraiseAnswer(Integer userId, Integer answerId) {
        Integer row = answerMapper.delAnswerFabulous(userId, answerId);
        if (row==0) {
            return new JsonResult(406,"你已经取消点赞，请刷新试试！");
        }
